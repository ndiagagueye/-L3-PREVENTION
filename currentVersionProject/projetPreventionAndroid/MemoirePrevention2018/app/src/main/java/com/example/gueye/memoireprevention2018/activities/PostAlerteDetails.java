...
import com.example.gueye.memoireprevention2018.adaptaters.LikesRecyclerAdapter;
...
import static com.example.gueye.memoireprevention2018.utils.Const.DEFAULT_TYPES;
...
public class PostAlerteDetails extends AppCompatActivity implements OnMapReadyCallback ,GoogleApiClient.OnConnectionFailedListener{
    ...
    private  Toolbar post_details_toolbar;
    ...
    private void initialise() {
        ...
        tvCommentCount = findViewById( R.id.comment_count );
        tvLikeCount = findViewById( R.id.like_count );
        icon_like = findViewById( R.id.icon_like );
        post_details_toolbar = findViewById( R.id.post_details_toolbar );
    ...
    private void showCurrentPost() {
        ...
        int type = getIntent().getIntExtra(Const.TYPE,8);
        String date = getIntent().getStringExtra(Const.DATE);
        postId = getIntent().getStringExtra(Const.POST_ID);
        currentUserId = getIntent().getStringExtra(Const.USER_ID);
        ...
        if(!postImage.equals("")){
            ...
            ivPostImage.setImageResource(Const.DEFAULT_RESOURCE_IMAGES[type]);

        }

        tvTypePost.setText(DEFAULT_TYPES[type]);
        ...
        firebaseFirestore.collection("Posts").document(postId).collection("Comments").addSnapshotListener( new EventListener<QuerySnapshot>() {
            ...
            public void onEvent(QuerySnapshot documentSnapshots, FirebaseFirestoreException e) {
                ...
                if (!documentSnapshots.isEmpty()){
                    ...
                    tvCommentCount.setText(""+countComment);
                }else{
        ...
        setupRecycleViewsComment(postId);
        setupRecycleViewsLike(postId);
    ...
    private void setupRecycleViewsComment(String blogPostId) {
    ...
    private void setupRecycleViewsLike(String blogPostId) {

        likesList = new ArrayList<>();
        likesRecyclerAdapter = new LikesRecyclerAdapter(likesList);
        ...
        firebaseFirestore.collection("Posts/" + blogPostId + "/Likes").orderBy("timestamp", Query.Direction.DESCENDING)
                .addSnapshotListener(PostAlerteDetails.this, new EventListener<QuerySnapshot>() {
                    @Override
                    public void onEvent(QuerySnapshot documentSnapshots, FirebaseFirestoreException e) {
                        ...
                        if (!documentSnapshots.isEmpty()) {
                            ...
                            for (DocumentChange doc : documentSnapshots.getDocumentChanges()) {
                                ...
                                if (doc.getType() == DocumentChange.Type.ADDED) {
                                    ...
                                    Like like = doc.getDocument().toObject(Like.class).withId(likeId);
                                    likesList.add(like);
                                    likesRecyclerAdapter.notifyDataSetChanged();

                                    if (like.getUser_id().equals(currentUserId)){
                                        icon_like.setImageDrawable(getApplicationContext().getDrawable(R.drawable.ic_like_red));

                                    }

                                    rvLikes.setAdapter(likesRecyclerAdapter);
